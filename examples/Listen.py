...
try:
    from sophiaii_sdk import SophiaIIISDK
except ImportError:
    subprocess.check_call([sys.executable, "-m", "pip", "install", "sophiaii_sdk"])
    from sophiaii_sdk import SophiaIIISDK
...
logging.basicConfig(
    ...
    filename="sophiaii_listen.log",
)


class SophiaIIListen:
    def __init__(
        self,
        server="http://localhost:7437",
        api_key="",
        agent_name="nora_ma-01",
        conversation_name="",
        whisper_model="base.en",
        wake_word="hey assistant",
    ):
        self.sdk = SophiaIIISDK(base_uri=server, api_key=api_key)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Sophia III Voice Assistant with Continuous Recording"
    )
    parser.add_argument(
        "--server", default="http://localhost:7437", help="Sophia III server URL"
    )
    parser.add_argument("--api_key", default="", help="Sophia III API key")
    parser.add_argument("--agent_name", default="nora_ma-01", help="Name of the agent")
    parser.add_argument(
    ...
    try:
        ...
        logging.error(f"Error initializing or running SophiaIIListen: {str(e)}")
        logging.debug(traceback.format_exc())
